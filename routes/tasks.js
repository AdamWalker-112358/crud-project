...
router.put('/', async (request, response, next) => {
    try {
        const task = await database.update('tasks', request.body, request.body.id, OVERWRITE)
        response.status(200).json(task);
    } catch (error) {
        next(error) 
    }
...
router.patch('/', async (request, response, next) => {
 try {
     const task = await database.update('tasks', request.body, request.body.id, MERGE)
     response.status(200).json(task);
 } catch (error) {
     next(error)
 }
